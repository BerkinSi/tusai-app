...
export default function DashboardPage() {
  const { user, profile, authState, loading } = useAuth();
  ...
  useEffect(() => {
    ...
    console.log('Dashboard: Profile:', profile?.full_name);
    console.log('Dashboard: Loading:', loading);
  }, [authState, user, profile, loading]);
  ...
  useEffect(() => {
    ...
    if (authState === 'authenticated' && user && profile?.id) {
      ...
      console.log('Dashboard: Not fetching data because:', {
        authStateNotAuthenticated: authState !== 'authenticated',
        noUser: !user,
  ...
  const fetchNotes = async () => {
    console.log('Dashboard: fetchNotes called');
    ...
    try {
      ...
      console.log('Dashboard: Notes API response status:', response.status);
      if (response.ok) {
        const notesData = await response.json();
        console.log('Dashboard: Notes data received:', notesData.length, 'notes');
        setNotes(notesData.slice(0, 3)); // Show only first 3 notes
      } else {
        console.error('Dashboard: Notes API error:', response.status, response.statusText);
      }
    } catch (error) {
  ...
  const shouldShowFullDashboard = authState === 'authenticated' && user && profile;
  const isLoading = loading || authState === 'loading' || authState === 'idle';
    ...
    return (
      <div className="w-full max-w-5xl mx-auto px-2 sm:px-4 py-6">
        <div className="text-center py-8">
          ...
          <p className="text-gray-600 mb-6">Dashboard'u görüntülemek için giriş yapın.</p>
          <Link 
            href="/giris" 
    ...
    return (
      <div className="w-full max-w-5xl mx-auto px-2 sm:px-4 py-6">
        <div className="text-center py-8">
          ...
          <p className="text-gray-600 mb-6">Bir hata oluştu. Lütfen sayfayı yenileyin.</p>
          <button 
  ...
  return (
    <div className="w-full max-w-5xl mx-auto px-2 sm:px-4 py-6 flex flex-col gap-8">
      ...
      <section className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <button 
          onClick={() => router.push("/quiz/new?mode=karma")}
        ...
        <button 
          onClick={() => router.push("/quiz/new?mode=zayif")}
        ...
        <button 
          onClick={() => router.push("/quiz/new?mode=cikmis")}
